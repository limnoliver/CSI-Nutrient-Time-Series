...
## for chlorophyll model, also include TN, TP, TN:TP slopes
...
rf.run.lake.blup <- function(response, dat) {
  
  #filter data frame to only response and predictors 
  
  vars.keep = grep(paste("^",response, "_", sep = ""), names(dat))
  vars.keep = vars.keep[c(4:7)]
  dat.keep = dat[,c(16:17,19:21,23:26,40:46,vars.keep)]
  dat.keep = dat.keep[complete.cases(dat.keep), ]
  
  pred.vars = names(dat.keep)[names(dat.keep) != paste(response, "_blup", sep = "")]
  response.var = paste(response, "_blup", sep = "")
...
tn.rf.lake.blup = rf.run.lake.blup("tn", lake.predictors)
tp.rf.lake.blup = rf.run.lake.blup("tp", lake.predictors)
tntp.rf.lake.blup = rf.run.lake.blup("tntp", lake.predictors)
chl.rf.lake.blup = rf.run.lake.blup("chl", lake.predictors)
...
rf.run <- function(response, dat) {
  
  #filter data frame to only response and predictors 
  
  vars.keep = grep(paste("^",response, "_", sep = ""), names(dat))
  #vars.keep = vars.keep[c(1,4,5,6)]
  vars.keep = vars.keep[1]
  dat.keep = dat[,c(17:21,23:26,40:67, vars.keep)]

  dat.keep = dat.keep[complete.cases(dat.keep), ]
  
  pred.vars = names(dat.keep)[names(dat.keep) != paste(response, "_coef_mean", sep = "")]
  response.var = paste(response, "_coef_mean", sep = "")
...
tn.rf = rf.run("tn", lake.predictors)
tp.rf = rf.run("tp", lake.predictors)
tntp.rf = rf.run("tntp", lake.predictors)
chl.rf = rf.run("chl", lake.predictors)

#modify vars.keep and rerun

tn.rf.eco = rf.run("tn", lake.predictors)
tp.rf.eco = rf.run("tp", lake.predictors)
tntp.rf.eco = rf.run("tntp", lake.predictors)
chl.rf.eco = rf.run("chl", lake.predictors)

# run the rf analysis on cateogrical response: increasing, decreasing, not changing
# need to weight responses since groups are not equally represented
...
rf.run.lake <- function(response, dat) {
...
tn.rf.lake = rf.run.lake("tn", lake.predictors)
tp.rf.lake = rf.run.lake("tp", lake.predictors)
tntp.rf.lake = rf.run.lake("tntp", lake.predictors)
chl.rf.lake = rf.run.lake("chl", lake.predictors)

#modify vars.keep and rerun
tn.rf.lake.eco = rf.run.lake("tn", lake.predictors)
tp.rf.lake.eco = rf.run.lake("tp", lake.predictors)
tntp.rf.lake.eco = rf.run.lake("tntp", lake.predictors)
chl.rf.lake.eco = rf.run.lake("chl", lake.predictors)

rf.run.region <- function(response, dat) {
  ...
  dat.keep = dat[,c(46:67,vars.keep)]
  dat.keep = dat.keep[complete.cases(dat.keep), ]
...
tn.rf.region = rf.run.region("tn", lake.predictors)
tp.rf.region = rf.run.region("tp", lake.predictors)
tntp.rf.region = rf.run.region("tntp", lake.predictors)
chl.rf.region = rf.run.region("chl", lake.predictors)

## now run rf on lake and region BLUPS 

## first merge region blups with region predictors
names(chag)
region.blups = merge(region.blups, hu4.predictors, by = "hu4_zoneid", all.x = TRUE)

rf.run.region.blup <- function(response, dat) {
  
  #filter data frame to only response and predictors 
  
  vars.keep = grep(paste("^",response, "_", sep = ""), names(dat))
  dat.keep = dat[,c(6:31,vars.keep)]
  dat.keep = dat.keep[complete.cases(dat.keep), ]
...
tn.rf.region.blup = rf.run.region.blup("tn", region.blups)
tp.rf.region.blup = rf.run.region.blup("tp", region.blups)
tntp.rf.region.blup = rf.run.region.blup("tntp", region.blups)
chl.rf.region.blup = rf.run.region.blup("chl", region.blups)

# create function that extracts top five variables from each model

temp = varImpPlot(tn.lake.rf)

# run chlorophyll model with tn, tp, and tn:tp in model

response = "chl_coef_mean"
predictors = lake.predictors[,c(5,16,17,19:22,24:27,31,35,39)]
rf.formula <- as.formula(paste(paste(response, "~"), paste(names(predictors), collapse= "+")))
chl.lake.rf <-randomForest(rf.formula,data=lake.predictors,na.action=na.omit,importance=1, ntree=2000)
...
# put together a dataframe with the top var in each model

top.vars = data.frame(Response_Variable = c("tn", "tp", "tntp", "chl"),
                      Predictor_Variable = c(row.names(varImpPlot(tn.rerun))[order(as.numeric(varImpPlot(tn.rerun)[,1]), decreasing = TRUE)[1]],
                                             row.names(varImpPlot(tp.rerun))[order(as.numeric(varImpPlot(tp.rerun)[,1]), decreasing = TRUE)[1]],
                                             row.names(varImpPlot(tntp.rerun))[order(as.numeric(varImpPlot(tntp.rerun)[,1]), decreasing = TRUE)[1]],
                                            "tmean.2011"),
                      Response_fullname = c("TN Change", "TP Change", "TN:TP Change", "Chl Change"),
                      Predictor_fullname = c("HUC4 TN Deposition 1990", "HUC4 Precipitation Change", "HUC4 Road Density", "HUC4 Mean Temperature 2011"))
                                             

tn.imp = data.frame(Response_Variable = "tn", 
                    Predictor_Variable = 
                    Per_inc_MSE = (as.numeric(varImpPlot(tn.rerun)[1,1])
...
save.image("C:/Users/Samantha/Dropbox/Time series/18AUG16.RData")
