...
plot(slope~blup.tn.stand$lagoslakeid[[2]])
plot(intercept~blup.tn.stand$lagoslakeid[[1]])
length(which(p.val<=0.05))
plot(intercept~slope)
plot(tn_umol_stand~sampleyear_cor, data = modern.15[modern.15$lagoslakeid==40610, ])
r.squaredGLMM(tn.model)
#check fit of model parameters by plotting data and estimated slope/intercept
plot(modern.15$tn_tp_umol[modern.15$lagoslakeid == 640]~modern.15$sampleyear_cor[modern.15$lagoslakeid == 640])
abline(a = 114, b = -1.479)

test = lm(modern.15$tn_tp_umol[modern.15$lagoslakeid == 640]~modern.15$sampleyear_cor[modern.15$lagoslakeid == 640])


blup.tn = coef(tn.model)
blup.tp = coef(tp.model)
blup.tntp = coef(tntp.model)
blup.se = se.ranef(tntp.model)

plot(blup.tntp$lagoslakeid[,2]~blup.tn$lagoslakeid[,2], xlab= "Change in TN (umol/year)", ylab = "Change in TP (umol/year)", xlim = c(-4,6))
plot(blup.tntp$lagoslakeid[,2]~blup.tp$lagoslakeid[,2])

pdf("Change_TNTP.pdf")
plot(blup.tn$lagoslakeid[,2]~blup.tp$lagoslakeid[,2], 
     pch = 16, cex.lab=2,cex=2,
     xlim=c(-0.05, .1), bty="l",xlab = "Change in TP (umol/year)", ylab = "Change in TN (umol/year)")
abline(0,0, col="red")
abline(v=0, col="red")
dev.off()
